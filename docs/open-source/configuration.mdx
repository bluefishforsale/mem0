...
## Unknown keys are rejected (Python)

`MemoryConfig` accepts only the keys it defines. Anything else fails validation
rather than being dropped:

```python
Memory.from_config({"add_context_topk": 3})
# pydantic_core.ValidationError: 1 validation error for MemoryConfig
# add_context_topk
#   Extra inputs are not permitted
```

That is deliberate. A misspelt knob that is silently ignored looks exactly like
one that is working, and so does a block the SDK no longer honours — `graph_store`
being the case people actually hit. Failing at the config is the only point where
the mistake is still cheap to find.

The self-hosted server applies the same rule: `POST /configure` answers `400` with
the offending key, and the running configuration is left as it was.
...
- `Extra inputs are not permitted` (Python): a config key is misspelt or no longer supported. The message names it; check it against the fields on `MemoryConfig`.
