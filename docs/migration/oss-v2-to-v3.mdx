...
- Remove the `graph_store` / `graphStore` block: `MemoryConfig` rejects it
...
<Note>
Leaving either in place is an error, not a silent no-op. `MemoryConfig` rejects
any key it does not define, so a config carrying `graph_store` fails where it is
written rather than starting up and quietly never doing graph memory.
</Note>
